...
void set_visit_node(Set set, BTreeNode node, VisitFunc visit) {
	...
	if (node->children[0] == NULL &&
		...
		for (int i = 0; i < node->count; i++)
				visit(set_node_value(set, node->set_nodes[i]));
	else {
		for (int i = 0; i < node->count; i++) {
			if (node->children[i] != NULL)
				set_visit_node(set, node->children[i], visit);
			visit(set_node_value(set, node->set_nodes[i]));
		}
		if (node->children[node->count] != NULL)
				set_visit_node(set, node->children[node->count], visit);
	}
}

void set_visit(Set set, VisitFunc visit) {

	set_visit_node(set, set->root, visit);
}
...
void set_visit(Set set, VisitFunc visit) {
	...
	visit(set_node_value(set, set_last(set)));
}
*/