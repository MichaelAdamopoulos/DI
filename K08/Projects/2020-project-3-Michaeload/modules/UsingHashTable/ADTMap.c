...
// Χρησιμοποιούμε seperate chaining, οπότε σύμφωνα με την θεωρία, πρέπει πάντα να διατηρούμε
...
int compare_map_nodes(Pointer a, Pointer b) {
	return ((MapNode)a)->owner->compare(((MapNode)a)->key, ((MapNode)b)->key);
}

Map map_create(CompareFunc compare, DestroyFunc destroy_key, DestroyFunc destroy_value) {
	// Δεσμεύουμε κατάλληλα τον χώρο που χρειαζόμαστε για το hash table
	Map map = malloc(sizeof(*map));
	map->capacity = prime_sizes[10];
	map->array = malloc(map->capacity * sizeof(List));

	// Δημιουργούμε τις κενές λίστες
	for (int i = 0; i < map->capacity; i++)
		map->array[i] = list_create(NULL);			// fix : null
...
void map_insert(Map map, Pointer key, Pointer value) {
	...
	MapNode found = (MapNode)list_find(map->array[pos], node, compare_map_nodes);

	if (found == NULL)
		list_insert_next(map->array[pos], LIST_EOF, node);
	else {
		found->key = key; //plus all the others
	}
...
MapNode map_find_node(Map map, Pointer key) {
	...
	return (MapNode)list_find(map->array[pos], node, compare_map_nodes);// to return eof
}
