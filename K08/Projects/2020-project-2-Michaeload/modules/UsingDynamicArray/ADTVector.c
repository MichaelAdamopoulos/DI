...
struct vector {
	...
	int steps;					// steps of last called function
};
...
Vector vector_create(int size, DestroyFunc destroy_value) {
	...
	vec->steps = 1;			// vector_create needs 1 step
...
void vector_insert_last(Vector vec, Pointer value) {
	...
	vec->steps = 1;		// 1 step for the inserted element
	if (vec->capacity == vec->size) {
		vec->steps += vec->size;		// realloc copies 'vec->size' elements
...
void vector_remove_last(Vector vec) {
	...
	//
	vec->steps = 1;		// 1 step for the removed element
...
Pointer vector_find(Vector vec, Pointer value, CompareFunc compare) {
	...
	for (int i = 0; i < vec->size; i++) {
		vec->steps++;	// 1 step for each compare
...
void vector_destroy(Vector vec) {
	...
	if (vec->destroy_value != NULL)
		for (int i = 0; i < vec->size; i++) {
			vec->steps++;	// 1 step for each destroy
